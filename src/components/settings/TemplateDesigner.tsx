...
export default function TemplateDesigner({ isOpen, onClose }: Props) {
  ...
  const loadLayout = async (
    tab: "shalwar_kameez" | "waistcoat" | "coat" = "shalwar_kameez",
  ) => {
    ...
    if (savedForTab && savedForTab.length > 0) {
      ...
      if (!hasShape6) {
        const shape6Default = defaultForTab.find(
          (el) => el.id === "svg_shape6",
        );
        if (shape6Default) {
  ...
  const handleReset = async () => {
    if (
      window.confirm(
        "Reset to default layout? All custom positioning will be lost.",
      )
    ) {
    ...
    switch (element.type) {
        ...
        if (element.id === "header_divider") {
          return (
            <div
              ...
              style={{ backgroundColor: element.color || "#cbd5e1" }}
            />
          );
        }
        const fontFamily =
      ...
      case "input": {
        ...
        const isSuitQtyHeaderValue =
          ...
          (element.content.hideLabel && element.content.field === "suitQty");

        return (
          <div
            className={`w-full h-full flex items-center bg-white/50 relative ${isHiddenHeaderValue ? "justify-center" : ""}`}
          >
            {!element.content.hideLabel && (
              <span
                className="font-semibold text-slate-600 text-[13px] px-1.5 shrink-0 whitespace-nowrap bg-white"
            ...
            {(element.content.inputs || []).map((inp: any, idx: number) => {
              ...
              return (
                <div
                  ...
                  dir="ltr"
                >
                  <span className="text-[10px] text-blue-700/80 px-1 whitespace-nowrap">
                    {inp.id}
                  </span>
                </div>
              );
      ...
      case "silaiGroup": {
        ...
        return (
          ...
          >
            <select
              className="py-1 px-2 text-xs border rounded text-center font-urdu bg-white shadow-sm pointer-events-none appearance-none w-fit"
              style={{
                fontFamily: "'NotoNastaliqUrdu', serif",
                maxWidth: "100%",
              }}
              dir="rtl"
            >
      ...
      case "banGroup": {
        const options = element.content?.options || [
          { key: "half_curved", labelUr: "ہاف بین گول" },
          { key: "half_straight", labelUr: "ہاف بین سیدھا" },
          { key: "full_curved", labelUr: "فل بین گول" },
          { key: "full_straight", labelUr: "فل بین سیدھا" },
        ];
        return (
          <div
            className="w-full h-full flex flex-col justify-center text-xs font-semibold text-slate-600 font-urdu px-2"
      ...
      case "wcCollarGroup": {
        const options = element.content?.options || [
          {
            key: "wc_collar_ban",
            asset: "WaistCoat/Ban_Collar.svg",
            raw: wcBanCollarRaw,
            labelUr: "بین کالر",
          },
          {
            key: "wc_collar_gol",
            asset: "WaistCoat/Gol_Collar.svg",
            raw: wcGolCollarRaw,
            labelUr: "گول کالر",
          },
          {
            key: "wc_collar_v",
            asset: "WaistCoat/V_Shaped.svg",
            raw: wcVShapedRaw,
            labelUr: "وی شیپ",
          },
        ];
        return (
          ...
          >
            <select
              className="py-3 px-2 text-[13px] leading-[2.2] border rounded text-center font-urdu bg-white shadow-sm pointer-events-none appearance-none w-fit"
              style={{
                fontFamily: "'NotoNastaliqUrdu', serif",
                maxWidth: "100%",
              }}
      ...
      case "skPattiKaajGroup": {
        return (
          <div className="w-full h-full flex flex-col justify-center gap-1.5 px-2 pointer-events-none">
            <div className="flex items-center gap-2 select-none">
              <input
                type="checkbox"
                checked
      ...
      case "skPattiTypeGroup": {
        return (
          <div className="w-full h-full flex items-center justify-end px-2 pointer-events-none">
            <div className="text-[13px] font-bold text-slate-700 font-urdu" dir="rtl">
              سادہ پٹی
            </div>
          </div>
        );
      }
      case "skAsteenShapeGroup": {
        return (
          <div className="w-full h-full flex items-center justify-end px-2 pointer-events-none">
            <div className="text-[13px] font-bold text-slate-700 font-urdu" dir="rtl">
              بغیر بکرم
  ...
  const handleExport = async () => {
    try {
      ...
      const allLayouts: Record<string, any[]> = {
        shalwar_kameez:
          activeTab === "shalwar_kameez"
            ? layout
            : settings?.slipLayout || DEFAULT_LAYOUT,
        waistcoat:
          activeTab === "waistcoat"
            ? layout
            : settings?.slipLayoutWaistcoat || DEFAULT_WAISTCOAT_LAYOUT,
        coat:
          activeTab === "coat"
            ? layout
            : settings?.slipLayoutCoat || DEFAULT_COAT_LAYOUT,
      };
      ...
      downloadAnchorNode.setAttribute(
        "download",
        `tailorpro_all_templates_${Date.now()}.json`,
      );
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (
          importedLayout.version === "1.2_combined" &&
          importedLayout.layouts
        ) {
          const settings = await db.settings.toCollection().first();
          ...
          const mergeLayouts = (
            importedArr: any[] | undefined,
            defaultArr: any[],
          ) => {
            if (!importedArr) return defaultArr;
            return defaultArr.map((defaultEl) => {
              if (defaultEl.isFixed) return defaultEl;
              const importedEl = importedArr.find(
                (el: any) => el.id === defaultEl.id,
              );
              return importedEl || defaultEl;
            });
          };

          updateData.slipLayout = mergeLayouts(
            importedLayout.layouts.shalwar_kameez,
            DEFAULT_LAYOUT,
          );
          updateData.slipLayoutWaistcoat = mergeLayouts(
            importedLayout.layouts.waistcoat,
            DEFAULT_WAISTCOAT_LAYOUT,
          );
          updateData.slipLayoutCoat = mergeLayouts(
            importedLayout.layouts.coat,
            DEFAULT_COAT_LAYOUT,
          );
          ...
          if (activeTab === "waistcoat")
            activeDefault = DEFAULT_WAISTCOAT_LAYOUT;
          ...
          const mergedLayout = activeDefault.map((defaultEl) => {
            ...
            const importedEl = importedLayout.find(
              (el: any) => el.id === defaultEl.id,
            );
            return importedEl || defaultEl;
          });
              ...
              return (
                <Rnd
                  ...
                  onDragStop={(_e, d) => {
                    ...
                    newLayout[index].y =
                      (d.y / (pageSize === "A4" ? 707 : 700)) * 100;
                    setLayout(newLayout);
                  }}
                  onResizeStop={(_e, _direction, ref, _delta, position) => {
                    const newLayout = [...layout];
                    newLayout[index].width = (ref.offsetWidth / 500) * 100;
                    newLayout[index].height =
                      (ref.offsetHeight / (pageSize === "A4" ? 707 : 700)) *
                      100;
                    newLayout[index].x = (position.x / 500) * 100;
                    newLayout[index].y =
                      (position.y / (pageSize === "A4" ? 707 : 700)) * 100;
                    setLayout(newLayout);
                  }}
              ...
              {selectedElement.type === "svg" && (
                <div className="space-y-4 pt-4 border-t border-gray-100">
                  <div className="flex flex-col gap-2 mb-4 p-3 bg-blue-50/50 rounded-lg border border-blue-100">
                    <label className="block text-xs font-semibold text-blue-800 uppercase tracking-wider">
                      Custom SVG Image
                    </label>
                    <input
                      type="file"
                    ...
                    <p className="text-[10px] text-slate-500 leading-tight">
                      Upload an SVG file to replace the default shape. SVGs will
                      be automatically themed.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                    {(selectedElement.content.inputs || []).map(
                      (inp: any, idx: number) => (
                        <div
                          key={idx}
                          className="bg-gray-50 p-2 rounded border border-gray-200 text-xs space-y-2"
                        >
                          <div className="flex justify-between items-center">
                            ...
                            <button
                              onClick={() => {
                                const newInputs =
                                  selectedElement.content.inputs.filter(
