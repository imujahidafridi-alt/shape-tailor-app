...
export default function Orders() {
    ...
    const handleQuickPrint = async (e: React.MouseEvent, order: Order) => {
        ...
        try {
            ...
            const layout = settings?.slipLayout || DEFAULT_LAYOUT;
            // Fetch worker names
    ...
    const loadCustomersMap = async () => {
        ...
        all.forEach((c: Customer) => { if (c.id) map[c.id.toString()] = c; });
        setCustomerMap(map);
    };
    ...
    const filteredOrders = orders.filter(order => {
        ...
        const matchesSearch = !searchQuery ||
            order.id?.toString().includes(cleanSearch) ||
            `#${order.id}`.includes(searchLower) ||
            customer?.id?.toString().toLowerCase().includes(cleanSearch) ||
            ...
            {loading ? (
                ...
                <div className="bg-slate-800 rounded-xl shadow-lg border border-slate-700 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-slate-200 min-w-[800px]">
                            <thead className="bg-slate-900/50 text-slate-400 text-sm uppercase font-semibold border-b border-slate-700">
                                <tr>
                                    <th className="px-6 py-4">ID</th>
                                    <th className="px-6 py-4">{t('customers.title')}</th>
                                    <th className="px-6 py-4"><Calendar className="w-4 h-4 inline-block me-1" />{t('orders.orderDate')}</th>
                                    <th className="px-6 py-4"><ShoppingBag className="w-4 h-4 inline-block me-1" />{t('orders.dueDate')}</th>
                                    <th className="px-6 py-4"><Wallet className="w-4 h-4 inline-block me-1" />{t('orders.advancePayment')}</th>
                            ...
                            <tbody className="divide-y divide-slate-700/50">
                                {filteredOrders.map((order) => {
                                    ...
                                    return (
                                        <tr
                                            key={order.id}
                                            className={`hover:bg-slate-750/50 transition-colors group cursor-pointer ${isOverdue ? 'bg-red-500/5' : ''}`}
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                                <div className="flex justify-center items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button
                                                        ...
                                                        className="text-slate-400 hover:text-cyan-400 p-2 rounded-md hover:bg-slate-700 transition-colors"
