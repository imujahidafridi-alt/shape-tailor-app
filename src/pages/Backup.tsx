    ...
    const handleExportCSV = async () => {
        try {
            ...
            const measurementMap = measurements.reduce((acc, m) => {
                acc[m.customerId.toString()] = m;
                return acc;
            }, {} as Record<string, CustomerMeasurement>);
            ...
            const rows = customers.map((c: Customer) => {
                ...
                const m = measurementMap[c.id!.toString()]?.fields || {};
                const d = measurementMap[c.id!.toString()]?.designOptions || {};
