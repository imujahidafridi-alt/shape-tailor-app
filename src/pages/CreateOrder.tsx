...
export default function CreateOrder() {
    ...
    const initialCustomerId = initialCustomerIdParam 
        ? (/^\d+$/.test(initialCustomerIdParam) ? parseInt(initialCustomerIdParam) : initialCustomerIdParam) 
        : '';

    const [formData, setFormData] = useState({
        customerId: initialCustomerId as string | number,
    ...
    const filteredCustomers = useMemo(() => {
        const cleanSearch = customerSearch.trim().toLowerCase().replace(/^#/, '');
        return customers.filter(
            (c) =>
                c.name.toLowerCase().includes(cleanSearch) ||
                c.phone.includes(cleanSearch) ||
                String(c.id).toLowerCase().includes(cleanSearch) ||
                `#${c.id}`.toLowerCase().includes(customerSearch.trim().toLowerCase())
        );
        ...
        try {
            ...
            if (shouldPrint) {
                ...
                if (customer && measurement && savedOrder) {
                    ...
                    const html = generateMeasurementSlipHTML(customer, measurement, layout, settings, workerNames, savedOrder);
