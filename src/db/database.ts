...
export interface Settings {
    ...
    slipLayout?: any[]; // Custom layout array for the template editor (Shalwar Kameez)
    slipLayoutCoat?: any[]; // Custom layout array for Coat
    slipLayoutWaistcoat?: any[]; // Custom layout array for Waistcoat
...
class TailorProDB extends Dexie {
    public customers!: Table<Customer, number | string>;
    ...
    constructor() {
        ...
        // Upgrade to Version 2: Remove unique constraint on phone
        this.version(2).stores({
            customers: '++id, name, phone, createdAt' // phone is no longer unique (&phone -> phone)
        }).upgrade(async () => {
            // No data transformation required, just removing the unique index constraint
        });
    }
}
