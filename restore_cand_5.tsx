...
import { LayoutElement, DEFAULT_LAYOUT, DEFAULT_WAISTCOAT_LAYOUT, DEFAULT_COAT_LAYOUT, DAMAN_OPTIONS, SILAI_OPTIONS, BAN_OPTIONS, WC_COLLAR_OPTIONS, WC_GERA_OPTIONS, COAT_BUTTON_OPTIONS, COAT_CHAAK_OPTIONS, CUFF_OPTIONS, BUTTON_DESIGN_OPTIONS } from '@/utils/slipLayout';
import asset12GhumRaw from '/SVG/Asset_12_Ghum.svg?raw';

interface CustomerMeasurementFormProps {
    customerId: number | string;
    customerName?: string;
    onPrint?: (measurement: CustomerMeasurement, layoutToPrint: LayoutElement[]) => void;
    onPreview?: (measurement: CustomerMeasurement, layoutToPreview: LayoutElement[]) => void;
}
...
export default function CustomerMeasurementForm({
    ...
    const [activeGarment, setActiveGarment] = useState<'shalwar_kameez' | 'waistcoat' | 'coat'>('shalwar_kameez');
    ...
    const handleContextMenu = useCallback((e: React.MouseEvent, fieldKey: string, options: any[]) => {
        e.preventDefault();
        setContextMenu({
            visible: true,
            x: e.clientX,
    ...
    async function loadMeasurementsAndLayout() {
        ...
        try {
            ...
            if (settings) {
                ...
                const normalizedSavedLayout = (savedForGarment && savedForGarment.length > 0 ? savedForGarment : defaultForGarment).map((el: any) => {
    ...
    function confirmReset() {
        ...
        toast.success(isUrdu ? 'ناپ صاف کر دیئے گئے' : 'Measurements cleared successfully');
    }
                ...
                <h3 className="text-lg font-bold text-gray-800 flex items-center gap-3">
                    ...
                    {saveStatus === 'saving' && (
                        <span className="text-xs font-normal text-gray-500 flex items-center gap-1 animate-pulse bg-gray-100 px-2 py-1 rounded-full">
                            <Save className="w-3 h-3" /> {isUrdu ? 'Ù…Ø­ÙÙˆØ¸ ÛÙˆ Ø±ÛØ§ ÛÛ’...' : 'Saving...'}
                        </span>
                    )}
                    {saveStatus === 'saved' && (
                        <span className="text-xs font-normal text-green-600 flex items-center gap-1 bg-green-50 px-2 py-1 rounded-full">
                            <CheckCircle className="w-3 h-3" /> {isUrdu ? 'Ù…Ø­ÙÙˆØ¸' : 'Saved'}
                        </span>
                    )}
                    {saveStatus === 'error' && (
                ...
                >
                    {layout.map((element) => {
                        ...
                        if (element.type === 'textArea') {
                            return (
                                <div key={element.id} style={style} className="flex flex-col bg-white/50">
                        ...
                        if (element.type === 'input') {
                            ...
                            const inputClass = `${isHiddenHeaderValue ? 'w-full min-w-0' : 'flex-1 min-w-0 w-full'} outline-none text-slate-900 font-bold px-1.5 text-sm bg-transparent z-10 ${element.content.hideLabel ?
                                (isHiddenHeaderValue ? 'text-center text-base' : 'text-left text-base')
                                : ''
                                } ${['sNo', 'customerName'].includes(element.content.field) ? 'cursor-not-allowed opacity-80' : ''}`;

                            return (
                                <div key={element.id} style={style} className={`flex items-center ${isHiddenHeaderValue ? 'justify-center' : ''}`}>
                                    {!element.content.hideLabel && (
                                    ...
                                    <div style={isSuitQtyHeaderValue ? { width: '84%', margin: '0 auto' } : { width: '100%' }}>
                                        <input
                                            type="text"
                                            value={
                                            ...
                                            onChange={(e) => handleFieldChange(element.content.field, e.target.value)}
                                            readOnly={element.content.field === 'sNo' || element.content.field === 'customerName'}
                                            className={inputClass}
                                            style={{ fontFamily: 'Arial, sans-serif', textAlign: isHiddenHeaderValue ? 'center' : 'left', paddingLeft: isSuitQtyHeaderValue ? 0 : undefined, paddingRight: isSuitQtyHeaderValue ? 0 : undefined }}
                                            dir="ltr"
                                        />
                                    </div>
                        ...
                        if ((element.id === 'svg_shape13' || element.type === 'banGroup') && fields['sk_collar_type'] === 'simple_collar') {
                            return null;
                        }
                        // If 'ban_collar' is selected, DO NOT render Simple Collar Shape (14)
                        if (element.id === 'svg_shape14' && (fields['sk_collar_type'] === 'ban_collar' || !fields['sk_collar_type'])) {
                        ...
                        if (element.type === 'svg') {
                            ...
                            if (element.id === 'svg_shape12' && fields['sk_patti_type'] === 'ghum_patti') {
                        ...
                        if (element.type === 'cuffGroup') {
                            ...
                            const selectedLabel = val ? (options.find((o:any)=>o.key===val)?.labelUr || options.find((o:any)=>o.key===val)?.label) : 'کف کا انتخاب';
                            return (
                                <div key={element.id} style={{ ...style, width: 'fit-content' }} className="flex flex-col justify-center text-[13px] font-bold text-slate-700 font-urdu px-2 cursor-context-menu pointer-events-auto hover:text-primary-600 transition-colors drop-shadow-sm select-none" onContextMenu={(e) => handleContextMenu(e, 'cuff_selected', options)} title={isUrdu ? 'تبدیل کرنے کے لئے رائٹ کلک کریں' : 'Right-click to change'} dir="rtl">
                                    {selectedLabel}
                                </div>
                            );
                        } 
 ...
 if (element.type === 'buttonDesignGroup') {
                            ...
                            const selectedLabel = val ? (options.find((o:any)=>o.key===val)?.labelUr || options.find((o:any)=>o.key===val)?.label) : 'بٹن ڈیزائن کا انتخاب';
                            return (
                                <div key={element.id} style={{ ...style, width: 'fit-content' }} className="flex flex-col justify-center text-[13px] font-bold text-slate-700 font-urdu px-2 cursor-context-menu pointer-events-auto hover:text-primary-600 transition-colors drop-shadow-sm select-none" onContextMenu={(e) => handleContextMenu(e, 'button_design_selected', options)} title={isUrdu ? 'تبدیل کرنے کے لئے رائٹ کلک کریں' : 'Right-click to change'} dir="rtl">
                                    {selectedLabel}
                                </div>
                            );
                        } 
 ...
 if (element.type === 'silaiGroup') {
                            ...
                            const selectedLabel = val ? (options.find((o:any)=>o.key===val)?.labelUr || options.find((o:any)=>o.key===val)?.label) : 'منتخب کریں';
                            return (
                                <div key={element.id} style={{ ...style, width: 'fit-content' }} className="flex flex-col justify-center text-[13px] font-bold text-slate-700 font-urdu px-2 cursor-context-menu pointer-events-auto hover:text-primary-600 transition-colors drop-shadow-sm select-none" onContextMenu={(e) => handleContextMenu(e, 'silai_selected', options)} title={isUrdu ? 'تبدیل کرنے کے لئے رائٹ کلک کریں' : 'Right-click to change'} dir="rtl">
                                    {selectedLabel}
                                </div>
                            );
                        } 
 ...
 if (element.type === 'banGroup') {
                            ...
                            const selectedLabel = val ? (options.find((o:any)=>o.key===val)?.labelUr || options.find((o:any)=>o.key===val)?.label) : 'کالر کا انتخاب';
                            return (
                                <div key={element.id} style={{ ...style, width: 'fit-content' }} className="flex flex-col justify-center text-[13px] font-bold text-slate-700 font-urdu px-2 cursor-context-menu pointer-events-auto hover:text-primary-600 transition-colors drop-shadow-sm select-none" onContextMenu={(e) => handleContextMenu(e, 'ban_selected', options)} title={isUrdu ? 'تبدیل کرنے کے لئے رائٹ کلک کریں' : 'Right-click to change'} dir="rtl">
                                    {selectedLabel}
                                </div>
                            );
                        } 
 ...
 if (element.type === 'wcCollarGroup') {
                            ...
                            const selectedLabel = val ? (options.find((o:any)=>o.key===val)?.labelUr || options.find((o:any)=>o.key===val)?.label) : 'کالر کا انتخاب';
                            return (
                                <div key={element.id} style={{ ...style, width: 'fit-content' }} className="flex flex-col justify-center text-[13px] font-bold text-slate-700 font-urdu px-2 cursor-context-menu pointer-events-auto hover:text-primary-600 transition-colors drop-shadow-sm select-none" onContextMenu={(e) => handleContextMenu(e, 'wc_collar_selected', options)} title={isUrdu ? 'تبدیل کرنے کے لئے رائٹ کلک کریں' : 'Right-click to change'} dir="rtl">
                                    {selectedLabel}
                                </div>
                            );
                        } 
 ...
 if (element.type === 'wcGeraGroup') {
                            ...
                            const selectedLabel = val ? (options.find((o:any)=>o.key===val)?.labelUr || options.find((o:any)=>o.key===val)?.label) : 'گیرا کا انتخاب';
                            return (
                                <div key={element.id} style={{ ...style, width: 'fit-content' }} className="flex flex-col justify-center text-[13px] font-bold text-slate-700 font-urdu px-2 cursor-context-menu pointer-events-auto hover:text-primary-600 transition-colors drop-shadow-sm select-none" onContextMenu={(e) => handleContextMenu(e, 'wc_gera_selected', options)} title={isUrdu ? 'تبدیل کرنے کے لئے رائٹ کلک کریں' : 'Right-click to change'} dir="rtl">
                                    {selectedLabel}
                                </div>
                            );
                        } 
 ...
 if (element.type === 'coatDoublePressGroup') {
                            return (
                                <div key={element.id} style={style} className="flex items-center gap-3 px-2">
                                    <label className="flex items-center gap-2 cursor-pointer select-none">
                                        <input
                                            type="checkbox"
                                            checked={fields['coat_double_press'] === 'yes'}
                                            onChange={(e) => handleFieldChange('coat_double_press', e.target.checked ? 'yes' : 'no')}
                        ...
                        if (element.type === 'coatButtonGroup') {
                            ...
                            return (
                                <div key={element.id} style={style} className="flex items-center gap-4 px-2">
                                    {options.map((opt) => (
                                        <label key={opt.key} className="flex items-center gap-1.5 cursor-pointer select-none">
                                            <input
                                                ...
                                                onChange={() => handleFieldChange('coat_button_selected', opt.key)}
                                    ...
                                    {options.map((opt) => (
                                        <label key={opt.key} className="flex items-center gap-1.5 cursor-pointer select-none">
                                            <input
                                                ...
                                                onChange={() => handleFieldChange('coat_chaak_selected', opt.key)}
                                                className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300"
                                            />
                                            <span className="text-[13px] font-semibold text-slate-700 font-urdu leading-[2.2]" style={{ fontFamily: "'NotoNastaliqUrdu', serif" }} dir="rtl">{opt.labelUr}</span>
                                        </label>
                        ...
                        if (element.type === 'skCollarToggleGroup') {
                            ...
                            return (
                                <div key={element.id} style={style} className="flex flex-row items-center gap-3 px-2 bg-slate-50/50 p-1.5 rounded border border-slate-100 shadow-sm">
                                    <label className="flex items-center gap-1.5 cursor-pointer select-none">
                                        <input type="radio" name="sk_collar_type" value="ban_collar" checked={currentValue === 'ban_collar'} onChange={() => handleFieldChange('sk_collar_type', 'ban_collar')} className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300" />
                                    ...
                                    <label className="flex items-center gap-1.5 cursor-pointer select-none">
                                        <input type="radio" name="sk_collar_type" value="simple_collar" checked={currentValue === 'simple_collar'} onChange={() => handleFieldChange('sk_collar_type', 'simple_collar')} className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300" />
                        ...
                        if (element.type === 'skPattiKaajGroup') {
                            return (
                                <div key={element.id} style={{...style, zIndex: 20}} className="flex flex-col justify-center gap-1.5 px-2 pointer-events-auto">
                                    <label className="flex items-center gap-2 cursor-pointer select-none">
                                        <input
                                            type="checkbox"
                                            checked={fields['sk_patti_kaaj'] === 'yes'}
                                            onChange={(e) => handleFieldChange('sk_patti_kaaj', e.target.checked ? 'yes' : 'no')}
                                            className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
                                        />
                                    ...
                                    {/* context menu trigger for Sada / Ghum Patti */}
                                    <div 
                                        ...
                                        dir="rtl"
                                        onContextMenu={(e) => handleContextMenu(e, 'sk_patti_type', [{key: 'sada_patti', labelUr: 'سادہ پٹی'}, {key: 'ghum_patti', labelUr: 'گم پٹی'}])}
                                        title={isUrdu ? 'تبدیل کرنے کے لئے رائٹ کلک کریں' : 'Right-click to change'}
                                    >
                                        {fields['sk_patti_type'] === 'ghum_patti' ? 'گم پٹی' : 'سادہ پٹی'}
                                    </div>
                                </div>
                            );
                        }
